...
export function generateCrushFoldState(
  ...
  | undefined {
  const shouldAddStartCrush =
    node.prev_node_id === undefined && state.data?.startCrushFold;
  ...
  if (
    ...
    !angle ||
    !crushFoldDirValue ||
    !(shouldAddStartCrush || shouldAddEndCrush)
  )
    return undefined;
...
export function createCurshFoldD(
  ...
  const crushFoldState = generateCrushFoldState(node, to, state);
